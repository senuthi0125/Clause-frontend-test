            ...
            <div className="flex flex-col items-center gap-1.5">
              <div
                className={`flex h-9 w-9 items-center justify-center rounded-full border-2 text-sm font-semibold transition-all ${
                    ...
                    : "border-slate-300 bg-white text-slate-400 dark:border-white/20 dark:bg-white/5 dark:text-slate-500"
                }`}
              >
            ...
            {idx < STEPS.length - 1 && (
              <div
                className={`mx-2 mb-5 h-0.5 w-24 rounded-full transition-all md:w-36 ${
                  done ? "bg-green-400" : active ? "bg-blue-300" : "bg-slate-200 dark:bg-white/10"
                }`}
              />
            )}
          </div>
      ...
      <div
        ...
        className={`flex cursor-pointer flex-col items-center justify-center gap-4 rounded-2xl border-2 border-dashed p-14 transition-all ${
          dragging
            ...
            : "border-slate-300 bg-white hover:border-slate-400 hover:bg-slate-50 dark:border-white/20 dark:bg-white/5 dark:hover:border-white/30 dark:hover:bg-white/8"
        }`}
      ...
      >
        {uploading ? (
          <>
            ...
            <p className="text-base font-medium text-slate-700 dark:text-slate-300">Uploading your document…</p>
          </>
        ) : (
          <>
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-blue-50 dark:bg-blue-500/15">
...
function ReviewStep({
  ...
  const inputClass = "h-10 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm outline-none focus:border-blue-400 focus:bg-white dark:border-white/10 dark:bg-white/5 dark:text-white dark:placeholder:text-slate-500 dark:focus:border-blue-500 dark:focus:bg-white/10";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        {/* Contract meta */}
        <Card className="border border-slate-200 shadow-sm dark:border-white/10 dark:bg-white/5">
          <CardHeader>
          ...
          <CardContent>
            {analysing ? (
              <div className="flex flex-col items-center gap-3 py-8">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">Running AI analysis…</p>
              </div>
            ) : analysis ? (
              <div className="space-y-4">
                <div>
                  <div className="mb-1 flex items-center justify-between text-sm">
                    <span className="font-medium text-slate-700 dark:text-slate-300">Risk Score</span>
                ...
                {analysis.summary && (
                  <p className="rounded-xl bg-slate-50 p-3 text-sm text-slate-700 leading-relaxed dark:bg-white/5 dark:text-slate-300">
                    {analysis.summary}
                  </p>
                )}
                {(analysis.risk_factors?.length ?? 0) > 0 && (
                  <div>
                    <p className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Risk Factors</p>
                ...
                {(analysis.recommendations?.length ?? 0) > 0 && (
                  <div>
                    <p className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Recommendations</p>
                    <ul className="space-y-1">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">Analysis unavailable.</p>
            )}
          </CardContent>
        </Card>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
        <button
          ...
          className="flex w-full items-center justify-between px-5 py-4 text-left transition hover:bg-slate-50 dark:hover:bg-white/5"
        >
          <div className="flex items-center gap-2">
            <Edit3 className="h-4 w-4 text-blue-500" />
            <span className="font-medium text-slate-800 dark:text-white">Edit Contract Details</span>
            <span className="rounded-md bg-blue-50 px-2 py-0.5 text-xs text-blue-600 dark:bg-blue-500/15 dark:text-blue-400">
        ...
        {editOpen && (
          <div className="border-t border-slate-100 px-5 pb-5 pt-4 dark:border-white/10">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                ...
                <textarea
                  ...
                  className="w-full resize-none rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm outline-none focus:border-blue-400 focus:bg-white dark:border-white/10 dark:bg-white/5 dark:text-white dark:placeholder:text-slate-500 dark:focus:border-blue-500 dark:focus:bg-white/10"
                />
              </div>
            </div>
            ...
            <div className="mt-4 flex items-center justify-end gap-3">
              {saveMsg && (
                <span className={`text-sm font-medium ${saveMsg.startsWith("✓") ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>
                  {saveMsg}
                </span>
              )}
              <Button
...
function ConflictStep({
  ...
  if (hasConflicts) {
    return (
      <div className="space-y-5">
        {/* Alert banner */}
        <div className="flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 px-5 py-4 dark:border-red-500/20 dark:bg-red-500/10">
          <AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-red-600 dark:text-red-400" />
          <div>
            <p className="font-semibold text-red-800 dark:text-red-300">Conflict Detected</p>
        ...
        {/* Issues bar */}
        <div className="flex items-center justify-between rounded-xl border border-amber-200 bg-amber-50 px-5 py-3 dark:border-amber-500/20 dark:bg-amber-500/10">
          <div className="flex items-center gap-2">
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          ...
          <Card className="border border-slate-200 shadow-sm dark:border-white/10 dark:bg-white/5">
            <CardHeader>
              <CardTitle className="text-base dark:text-white">Your Uploaded Contract</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2 text-sm text-slate-600 dark:text-slate-400">
              ...
              {analysis?.summary && (
                <p className="mt-3 rounded-lg bg-slate-50 p-3 leading-relaxed dark:bg-white/5 dark:text-slate-300">
                  {analysis.summary}
                </p>
              )}
            </CardContent>
          ...
          <Card className="border border-red-100 shadow-sm dark:border-red-500/20 dark:bg-white/5">
            <CardHeader>
              ...
              {conflictResult.conflicts.map((c) => (
                ...
                >
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2">
                    <div className="rounded-lg bg-slate-50 p-2 text-xs dark:bg-white/5 dark:text-slate-300">
                      <span className="font-medium">Your clause:</span> {c.clause_a}
                    </div>
                    <div className="rounded-lg bg-red-50 p-2 text-xs dark:bg-red-500/10 dark:text-red-300">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col items-center gap-3 py-8">
        ...
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white">No Conflicts Detected!</h2>
      ...
      <Card className="border border-slate-200 shadow-sm dark:border-white/10 dark:bg-white/5">
        <CardHeader>
          <CardTitle className="dark:text-white">Contract Summary</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        />
        <ActionCard
          icon={<Plus className="h-7 w-7 text-slate-500 dark:text-slate-400" />}
...
function Row({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex items-start gap-2">
      <span className="w-28 shrink-0 font-medium text-slate-700 dark:text-slate-300">{label}:</span>
...
function SummaryRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div>
      ...
      <p className="mt-0.5 font-medium text-slate-800 dark:text-white">{value}</p>
    </div>
  );
}
...
function ActionCard({
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-2xl border border-slate-200 bg-white p-6 text-center shadow-sm dark:border-white/10 dark:bg-white/5">
      <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-slate-50 dark:bg-white/10">
        {icon}
      </div>
      <div>
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-4xl">
        ...
        {analysisError && (
          <div className="mb-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700 dark:border-amber-500/20 dark:bg-amber-500/10 dark:text-amber-300">
            {analysisError}
          </div>
        )}
